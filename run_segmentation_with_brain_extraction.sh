...
for input_image in "$INPUT_DIR"/*.nii.gz; do
    ...
    # Optional: Create probabilistic WM mask from posterior
    ThresholdImage 3 \
        "$subject_dir/${filename}_SegmentationPosteriors03.nii.gz" \
        "$subject_dir/${filename}_white_matter_prob.nii.gz" \
        0.5 1.0 1 0

...
def main():
    ...
    nc_output_dir = "../datasets/ADNI/t1_mpr_NC"
    
    # First identify all scan types
    # print("Identifying scan types...")
    # scans = find_scans(input_dir)
    # organize_scans_by_type(input_dir, output_dir, scans, 't1_mpr')
    
    # Then select Normal Controls from T1 MPR scans
    print("\nSelecting Normal Control scans...")
    nc_files = select_normal_controls(output_dir, nc_output_dir)
    print(f"Normal Control files found: {len(nc_files)}")
    print(f"Files copied to: {nc_output_dir}")
